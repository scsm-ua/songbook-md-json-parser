...
class Song {
    ...
    render() {
        ...
        if (json.word_by_word?.length) {
            json.word_by_word.forEach(t => {
                body += `> ${t}\n\n`;
            });
        }
        ...
        if (json.embeds?.length) {
            body += '\n';
            json.embeds.forEach(embed => {
                body += `[${embed.title}](${embed.embed_url})\n\n`;
            });
        }
...
function convertSongToJSON(text) {
    ...
    var heading_empty_line = false;

    // Push '' separator for empty line(s) between same-type heading lines.
    function pushHeadingSeparator(arr, line_id) {
        if (heading_empty_line && last_line_id === line_id && arr[arr.length - 1] !== '') {
            arr.push('');
        }
    }
    ...
    lines.forEach((line) => {
        ...
        switch (line_id) {
            case 'title':
                pushHeadingSeparator(song.title, 'title');
                song.title.push(line_value);
                break;
            case 'author':
                pushHeadingSeparator(song.author, 'author');
                song.author.push(line_value);
                break;
            case 'subtitle':
                if (song.verses.length === 0) {
                    pushHeadingSeparator(song.subtitle, 'subtitle');
                    song.subtitle.push(line_value);
                } else {
                    var verse = getLastVerse({ create_new: last_line_id !== 'subtitle' });
                    pushHeadingSeparator(verse.subtitle, 'subtitle');
            ...
            default:
                if (!line.trim()) {
                    ...
                    if (last_line_id === 'title' || last_line_id === 'subtitle' || last_line_id === 'author') {
                        heading_empty_line = true;
                    }
                } else {
